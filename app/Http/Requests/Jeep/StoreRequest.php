...
class StoreRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'price_type' => 'required|in:per_person,per_trip,per_vehicle,per_hour',
            ...
            'seasons.*.price_local' => ['required', 'numeric'],
            'seasons.*.price_arab' => ['required', 'numeric'],
            'seasons.*.price_foreigner' => ['required', 'numeric'],

            // Nationality prices
            'seasons.*.nationality_prices' => ['nullable', 'array'],
            'seasons.*.nationality_prices.*.nationality_id' => ['required', 'exists:nationalities,id'],
            'seasons.*.nationality_prices.*.price_type' => ['required', 'string'],
            'seasons.*.nationality_prices.*.price' => ['required', 'numeric'],
