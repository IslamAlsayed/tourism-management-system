<?php

return [
    'Change Language Not Successfully' => 'Change Language Not Successfully',
    'Change Language Successfully' => 'Change Language Successfully',
    'Failed to update language status. Please try again.' => 'Failed to Aggiorna Lingua status. Please try again.',
    'ably_key_not_configured' => 'Ably key not configured, skipping Ably broadcast.',
    'access_denied' => 'Access Denied',
    'accommodation_rates_records_info' => 'Accommodation price records will be automatically Cread for each combination (season × room) and (season × meal) with default values (0). You can modify the prices later.',
    'add_banner' => 'Add New Banner',
    'all_notifications_marked_read' => 'All Notifiche marked as read.',
    'are_you_sure' => 'Are you sure?',
    'are_you_sure_delete' => 'This item will be moved to the archive (soft Elimina). You can restore it later.',
    'are_you_sure_force_delete' => 'This will PERMANENTLY Elimina this record from the database.',
    'available_columns' => 'Available Columns',
    'backup_created_test' => 'Backup Cread Successofully (test mode).',
    'banner_image' => 'Banner Image',
    'cannot_delete_default_role' => 'Cannot Elimina default roles (superadmin, admin, user).',
    'capacity must be at least 1.' => 'Capacity must be at least 1.',
    'change_language_not_successfully' => 'Language change not Successoful.',
    'change_language_successfully' => 'Language changed Successofully.',
    'changes_saved' => 'Changes Saved',
    'city_created' => 'City Created',
    'city_creation_failed' => 'City Creation Failed',
    'city_deleted' => 'City Deleted',
    'city_deletion_failed' => 'City Deletion Failed',
    'city_update_failed' => 'City Update Failed',
    'city_updated' => 'City Updated',
    'closing time must be after opening time.' => 'Closing time must be after opening time.',
    'columns_cleared_to_minimum' => 'Columns cleared, keeping minimum required.',
    'columns_updated_successfully' => 'System columns Aggiornad Successofully.',
    'confirm_bulk_activate' => 'Are you sure you want to activate the selected items?',
    'confirm_bulk_deactivate' => 'Are you sure you want to deactivate the selected items?',
    'confirm_bulk_delete' => 'Are you sure you want to Elimina the selected items? NOTE: This is a',
    'confirm_bulk_disable_auto_update' => 'Are you sure you want to disable auto-Aggiorna (fix exchange rate) for the selected items?',
    'confirm_bulk_enable_auto_update' => 'Are you sure you want to enable auto-Aggiorna for the selected items?',
    'confirm_bulk_force_delete' => 'WARNING: Are you sure you want to PERMANENTLY Elimina the selected items? CRITICAL: This will completely erase the data from the database. Next time you Crea or Importa items, they will get NEW, higher IDs, which may break relationships with other modules (like Hotels or Restaurants). Use only if absolutely necessary!',
    'country_created' => 'Country Created',
    'country_creation_failed' => 'Country Creation Failed',
    'country_deleted' => 'Country Deleted',
    'country_deletion_failed' => 'Country Deletion Failed',
    'country_update_failed' => 'Country Update Failed',
    'country_updated' => 'Country Updated',
    'created_language_successfully' => 'Language Cread Successofully.',
    'created_not_language_successfully' => 'Language not Cread Successofully.',
    'currency_created' => 'Currency Created',
    'currency_creation_failed' => 'Currency Creation Failed',
    'currency_deleted' => 'Currency Deleted',
    'currency_deletion_failed' => 'Currency Deletion Failed',
    'currency_update_failed' => 'Currency Update Failed',
    'currency_updated' => 'Currency Updated',
    'data_refreshed_successfully' => 'Data refreshed Successofully.',
    'default_language_updated' => 'تم تحديث اللغة الافتراضية بنجاح.',
    'edit_banner' => 'Edit Banner',
    'error_occurred' => 'Error Occurred',
    'field_deleted_successfully' => ':field has been :status Successofully.',
    'field_updated_successfully' => ':field has been :status Successofully.',
    'force_delete_warning' => 'This Azione cannot be undone. All related data may be affected.',
    'general_error' => 'An Errore occurred. Please try again.',
    'goodbye_name' => 'Goodbye, :name!',
    'hold_ctrl_to_select_multiple' => 'Hold Ctrl (Windows) or Cmd (Mac) to select multiple items.',
    'invalid_credentials' => 'Invalid Credentials',
    'invalid_input' => 'Invalid Input',
    'invalid_model_specified' => 'Invalid model specified.',
    'invalid_type' => 'Invalid Type',
    'item_created' => 'Item Created',
    'item_deleted' => 'Item Deleted',
    'item_updated' => 'Item Updated',
    'language_deleted_successfully' => 'Language Eliminad Successofully.',
    'language_deletion_failed' => 'Language deletion Le credenziali fornite sono errate..',
    'latitude must be between -90 and 90.' => 'Latitude must be between -90 and 90.',
    'longitude must be between -180 and 180.' => 'Longitude must be between -180 and 180.',
    'manage_system_columns' => 'Manage System Columns',
    'model_not_found' => 'Model not found.',
    'no_backup' => 'No backup files found.',
    'no_items_selected' => 'No items selected.',
    'no_permissions_assigned' => 'No permissions assigned to this role.',
    'no_permissions_available' => 'No permissions available.',
    'no_photo_uploaded' => 'No Photo Uploaded',
    'no_records_found' => 'No records found.',
    'no_results_found' => 'No results found',
    'no_seasons_found' => 'No active seasons found.',
    'not_found' => 'Not Found',
    'not_found_this_type' => 'Not found this :type',
    'notification_deleted' => 'Notification Eliminad.',
    'notification_marked_read' => 'Notification marked as read.',
    'notification_marked_unread' => 'Notification marked as unread.',
    'operation_failed' => 'Operation Failed',
    'operation_successful' => 'Operation Successful',
    'page_banners' => 'Page Banners',
    'password_updated' => 'Password Updated',
    'permission_created_successfully' => 'Permission Cread Successofully.',
    'permission_creation_failed' => 'Permission creation Le credenziali fornite sono errate..',
    'permission_deleted_failed' => 'Permission deletion Le credenziali fornite sono errate..',
    'permission_deleted_successfully' => 'Permission Eliminad Successofully.',
    'permission_name_format' => 'Use snake_case format like: Crea_Utenti, Modifica_posts, Elimina_comments',
    'permission_updated_failed' => 'Permission Aggiorna Le credenziali fornite sono errate..',
    'permission_updated_successfully' => 'Permission Aggiornad Successofully.',
    'permissions_count' => 'Permissions count: :count',
    'photo_uploaded_successfully' => 'Photo Uploaded Successfully',
    'please_login_to_continue' => 'Please Accesso to continue.',
    'profile_photo_updated' => 'Profile Photo Updated',
    'profile_updated' => 'Profile Updated',
    'quick_edit_saved' => ':type Aggiornad Successofully.',
    'record_not_found' => 'Record not found.',
    'restaurant_created' => 'Restaurant Created',
    'restaurant_creation_failed' => 'Restaurant Creation Failed',
    'restaurant_deleted' => 'Restaurant Deleted',
    'restaurant_deletion_failed' => 'Restaurant Deletion Failed',
    'restaurant_update_failed' => 'Restaurant Update Failed',
    'restaurant_updated' => 'Restaurant Updated',
    'role_created_successfully' => 'Role Cread Successofully.',
    'role_creation_failed' => 'Role creation Le credenziali fornite sono errate..',
    'role_deleted_failed' => 'Role deletion Le credenziali fornite sono errate..',
    'role_deleted_successfully' => 'Role Eliminad Successofully.',
    'role_updated_failed' => 'Role Aggiorna Le credenziali fornite sono errate..',
    'role_updated_successfully' => 'Role Aggiornad Successofully.',
    'route' => 'Route',
    'select_at_least_one_notification_type' => 'Select at least one notification type.',
    'select_module' => 'Select Module',
    'select_route' => 'Select Route...',
    'selected_items_deleted' => 'Selected items Eliminad Successofully. :count items removed.',
    'server_error' => 'Server Error',
    'session_expired' => 'Session has expired. Please log in again.',
    'session_lifetime_min_5' => 'Session lifetime must be at least 5 minutes',
    'session_lifetime_min_5_or_0' => 'Session lifetime must be 0 or at least 5 minutes',
    'sidebar_export_error' => 'Error Esportaing sidebar config: :Errore',
    'sidebar_item_hidden' => 'Menu item hidden.',
    'sidebar_item_shown' => 'Menu item shown.',
    'sidebar_order_update_error' => 'Error updating sidebar order: :Errore',
    'sidebar_order_updated' => 'Sidebar order Aggiornad Successofully.',
    'sidebar_reset_default' => 'Sidebar reset to default order.',
    'sidebar_reset_error' => 'Error resetting sidebar: :Errore',
    'status_updated' => 'تم تحديث حالة :type بنجاح.',
    'system_columns' => 'System Columns Management',
    'the crossing port code must be unique.' => 'The Crossing/Port code must be unique.',
    'the crossing port name is required.' => 'The Crossing/Port name is required.',
    'the crossing port type is required.' => 'The Crossing/Port type is required.',
    'the selected crossing port type is invalid.' => 'The selected Crossing/Port type is invalid.',
    'type_created' => ':type Created',
    'type_created_by' => ':record_name has been Aggiungied to :type_name by :user_name',
    'type_creation_failed' => ':type Creation Failed',
    'type_deleted' => ':type Deleted',
    'type_deleted_by' => ':record_name from :type_name has been Eliminad by :user_name',
    'type_deleted_count' => ':count :type Deleted',
    'type_deletion_failed' => ':type Deletion Failed',
    'type_force_deleted' => ':type Force Deleted',
    'type_not_found' => 'Not found :type',
    'type_update_failed' => ':type Update Failed',
    'type_updated' => ':type Updated',
    'type_updated_by' => ':record_name from :type_name has been Aggiornad by :user_name',
    'type_updated_count' => ':count :type Updated',
    'update_not_successfully' => 'Update Not Successfully',
    'updated_successfully' => 'Updated Successfully',
    'user_created' => 'User Created',
    'user_creation_failed' => 'User Creation Failed',
    'user_deleted' => 'User Deleted',
    'user_deletion_failed' => 'User Deletion Failed',
    'user_logged_in' => ':name logged in.',
    'user_logged_out' => ':name logged out.',
    'user_session_expired' => ':name session expired.',
    'user_update_failed' => 'User Update Failed',
    'user_updated' => 'User Updated',
    'validation_error' => 'Validation Error',
    'verification_link_sent' => 'Verification link sent to your Email.',
    'welcome_back_name' => 'Welcome back, :name!',
    'you_logged_in' => 'You logged in Successofully.',
    'you_logged_out' => 'You logged out Successofully.',
];
