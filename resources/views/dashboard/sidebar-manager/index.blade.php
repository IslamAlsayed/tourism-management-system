...
.menu-item {
    ...
    border-right: 3px solid #3699ff0a;
    min-height: 38px;
}

.menu-item:hover {
    box-shadow: 0 2px 8px rgba(54, 153, 255, 0.10);
    border-color: #3699ff;
...
.menu-item.sortable-chosen {
    ...
    box-shadow: 0 4px 16px rgba(54, 153, 255, 0.13);
    z-index: 1000;
}
...
function initializeSortable() {
    ...
    if (menuList) {
        sortable = Sortable.create(menuList, {
            ...
            onEnd: function() {
                updateOrderNumbers();
                saveMenuOrder(); // حفظ تلقائي عند السحب والإفلات
                logActivity('تم تغيير ترتيب العناصر وحفظ الترتيب', 'success');
            }
        });
    }
}
