<div class="kt-card kt-card-grid min-w-full">
    @component('includes.pagination-info', [
        ...
        'entityName' => __('main.transportation-company-bus-types'),
        'showSearch' => true,
    ])
                            ...
                            <tr wire:key="{{ $transportationCompanyBusType->id }}" class="hover:bg-gray-100">
                                ...
                                @foreach ($columns as $column)
                                    @include('components.static-columns', [
                                        'column' => $column,
                                        'model' => $transportationCompanyBusType,
                                        'search' => $search,
                                    ])
                                @endforeach
                                <td class="px-4 py-2 text-end">
                                    <div>
                                        <a href="{{ route('transportation-company-bus-types.edit', $transportationCompanyBusType->id) }}"
