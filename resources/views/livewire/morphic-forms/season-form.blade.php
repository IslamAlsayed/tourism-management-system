<div>
    ...
    <div class="kt-card bg-blue-100">
        <div class="kt-card-header">
            ...
            <button type="button" wire:click="addSeason" toggle-button
        ...
        <div class="kt-card-body {{ count($seasons) > 0 ? 'p-4' : '' }}" wire:target="removeSeason"
                    ...
                    <div class="kt-card p-4 border-2" wire:key="season-{{ $index }}">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button type="button" wire:click="removeSeason({{ $index }})"
                                class="kt-btn kt-btn-sm bg-danger text-white {{ count($seasons) > 1 ? '' : 'hidden' }}"
                                toggle-button>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 items-end gap-6 mb-4">
                            ...
                            <div class="align-self-end">
                                ...
                                <input type="text" name="seasons[{{ $index }}][name]"
                                    id="seasons_{{ $index }}_name" class="kt-input h-[45px]"
                                    wire:model="seasons.{{ $index }}.name"
                                    value="{{ old('seasons.' . $index . '.name') }}">
                            ...
                            {{-- Name (Arabic) --}}
                            <div>
                                <label for="seasons_{{ $index }}_name_ar"
                                    class="kt-label mb-2">{{ __('main.name_ar') }}</label>
                                <input type="text" name="seasons[{{ $index }}][name_ar]"
                                    id="seasons_{{ $index }}_name_ar" class="kt-input h-[45px]"
                                    wire:model="seasons.{{ $index }}.name_ar"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 items-end gap-6 mb-4">
                            ...
                            <div class="align-self-end">
                                ...
                                <input type="date" name="seasons[{{ $index }}][season_from]"
                                    id="seasons_{{ $index }}_season_from" class="kt-input h-[45px]"
                                    wire:model="seasons.{{ $index }}.season_from"
                            ...
                            <div class="align-self-end">
                                ...
                                <input type="date" name="seasons[{{ $index }}][season_to]"
                                    id="seasons_{{ $index }}_season_to" class="kt-input h-[45px]"
                                    wire:model="seasons.{{ $index }}.season_to"
                        ...
                        {{-- description --}}
                        <div class="mb-4">
                            <label for="seasons_{{ $index }}_description"
                                class="kt-label mb-2">{{ __('main.description') }}</label>
                            <input id="seasons_{{ $index }}_description" type="hidden"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="custom-input">
                                    <input type="checkbox" name="seasons[{{ $index }}][is_active]"
                                        id="season-{{ $index }}-is_active"
                                        wire:model="seasons.{{ $index }}.is_active" value="1"
                                        {{ old('seasons.' . $index . '.is_active', 1) ? 'checked' : '' }}
                                        data-kt-datatable-row-check="true">
                                    <label
                                        for="season-{{ $index }}-is_active">{{ __('main.is_active') }}</label>
                                </div>
                            </div>
                        </div>
                    </div>
