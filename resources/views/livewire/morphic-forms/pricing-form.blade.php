<div>
    ...
    <div class="kt-card bg-orange-100">
        <div class="kt-card-header">
            ...
            <button type="button" wire:click="addPricing" toggle-button
                    ...
                    <div class="kt-card p-4 border-2" wire:key="pricing-{{ $index }}">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button type="button" wire:click="removePricing({{ $index }})"
                                class="kt-btn kt-btn-sm bg-danger text-white {{ count($pricings) > 1 ? '' : 'hidden' }}"
                                toggle-button>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 items-end gap-6 mb-4">
                            ...
                            <div class="align-self-end">
                                ...
                                <select name="pricings[{{ $index }}][vehicle_type_id]"
                                    id="pricings_{{ $index }}_vehicle_type_id" class="kt-select basic-single"
                                    wire:model.live="pricings.{{ $index }}.vehicle_type_id">
                                    <option value="" disabled selected></option>
                                    @foreach ($vehicleTypes as $id => $name)
                                        <option value="{{ $id }}"
                                            {{ old('pricings.' . $index . '.vehicle_type_id') == $id ? 'selected' : '' }}>
                                            {{ $name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="align-self-end">
                                ...
                                <input type="number" step="0.01" name="pricings[{{ $index }}][price]"
                                    id="pricings_{{ $index }}_price" class="kt-input h-[45px]"
                                    wire:model="pricings.{{ $index }}.price"
                            ...
                            <div class="align-self-end">
                                ...
                                <input type="number" step="1" minlength="1" maxlength="100"
                                    name="pricings[{{ $index }}][tax]" id="pricings_{{ $index }}_tax"
                                    class="kt-input h-[45px]" wire:model="pricings.{{ $index }}.tax"
                            ...
                            <div class="align-self-end">
                                ...
                                <select name="pricings[{{ $index }}][pricing_unit_id]"
                                    id="pricings_{{ $index }}_pricing_unit_id" class="kt-select basic-single"
                                    wire:model="pricings.{{ $index }}.pricing_unit_id">
                                    <option value="" disabled selected></option>
                                    @foreach ($currencies as $id => $code)
                                        <option value="{{ $id }}"
                                            {{ old('pricings.' . $index . '.pricing_unit_id') == $id ? 'selected' : '' }}>
                                            {{ $code }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="align-self-end">
                                ...
                                <select name="pricings[{{ $index }}][currency_id]"
                                    id="pricings_{{ $index }}_currency_id" class="kt-select basic-single"
                                    wire:model="pricings.{{ $index }}.currency_id">
                                    <option value="" disabled selected></option>
                                    @foreach ($currencies as $id => $code)
                                        <option value="{{ $id }}"
                                            {{ old('pricings.' . $index . '.currency_id') == $id ? 'selected' : '' }}>
                                            {{ $code }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        {{-- description --}}
                        <div class="mb-4">
                            <label for="pricings_{{ $index }}_description"
                                class="kt-label mb-2">{{ __('main.description') }}</label>
                            <input id="pricings_{{ $index }}_description" type="hidden"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="custom-input">
                                <input type="checkbox" name="pricings[{{ $index }}][is_active]"
                                    id="pricing-{{ $index }}-is_active"
                                    wire:model="pricings.{{ $index }}.is_active" value="1"
                                    {{ old('pricings.' . $index . '.is_active', 1) ? 'checked' : '' }}
