<div>
    ...
    <div class="flex flex-row-reverse flex-wrap items-center gap-2 p-4 w-full">
        <input type="text" wire:model.debounce.500ms="search" placeholder="{{ __('بحث عن بلد') }}"
            class="kt-input min-w-[180px]" />
        ...
        <!-- Show/Hide Columns -->
        <div class="relative">
            <button type="button" class="kt-btn kt-btn-outline flex items-center gap-1"
                onclick="document.getElementById('columnsDropdown').classList.toggle('hidden')">
                <i class="ki-filled ki-setting-4"></i> {{ __('إظهار/إخفاء الأعمدة') }}
            </button>
            <div id="columnsDropdown"
                class="absolute z-10 bg-white border rounded shadow p-3 mt-2 hidden min-w-[200px]">
                <label class="flex items-center gap-2 mb-1"><input type="checkbox" checked disabled> #</label>
                <label class="flex items-center gap-2 mb-1"><input type="checkbox" checked disabled>
                    {{ __('العلم') }}</label>
                ...
                <label class="flex items-center gap-2 mb-1"><input type="checkbox" checked>
                    {{ __('كود الهاتف') }}</label>
                ...
                <label class="flex items-center gap-2 mb-1"><input type="checkbox" checked>
                    {{ __('تاريخ الإنشاء') }}</label>
            </div>
        </div>
        <button class="kt-btn kt-btn-sm kt-btn-primary" wire:click="bulkEdit"
                        ...
                        <tr>
                            <td class="text-center">
                                <input class="kt-checkbox kt-checkbox-sm" data-kt-datatable-row-check="true"
                                    type="checkbox" value="{{ $country->id }}" wire:model="selected">
                                <div class="text-xs text-gray-400">{{ $country->id }}</div>
                            </td>
                            <td class="text-center">
                                @if ($country->flag_url)
                                    <img alt="" class="rounded-full size-7 mx-auto"
                            ...
                            <td>
                                <button wire:click="toggleActive({{ $country->id }})"
                                    class="px-2 py-1 rounded text-xs font-semibold {{ $country->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        ...
                                        <div class="kt-menu-dropdown kt-menu-default w-full max-w-[200px]"
                                            ...
                                            <div class="kt-menu-item">
                                                <a class="kt-menu-link" href="#"
                                                    onclick="event.preventDefault(); @this.call('makeCopy', {{ $country->id }})">
                                                    <span class="kt-menu-icon"><i
                                            ...
                                            <div class="kt-menu-item">
                                                <a class="kt-menu-link" href="#"
                                                    onclick="event.preventDefault(); if(confirm('{{ __('هل أنت متأكد من الحذف؟') }}')) { @this.call('delete', {{ $country->id }}) }">
                                                    <span class="kt-menu-icon"><i
                                            ...
                                            <div class="kt-menu-item">
                                                <a class="kt-menu-link" href="#"
                                                    onclick="event.preventDefault(); @this.call('toggleActive', {{ $country->id }})">
                                                    <span class="kt-menu-icon"><i
                                                            class="ki-filled {{ $country->is_active ? 'ki-cross' : 'ki-check' }}"></i></span>
                                                    <span
                                                        class="kt-menu-title">{{ $country->is_active ? __('تعطيل') : __('تفعيل') }}</span>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
        ...
        <div class="mt-8">
            <div class="kt-card">
                <div class="kt-card-header">
                    <h3 class="kt-card-title">{{ __('main.faq') }}</h3>
                </div>
                <div class="kt-card-body">
                    <ul class="list-disc pl-6">
                        <li>{{ __('main.how_to_add_new_country') }}</li>
                        <li>{{ __('main.how_to_edit_delete_country') }}</li>
                        <li>{{ __('main.how_to_use_bulk_edit') }}</li>
                        <li>{{ __('main.content_appears_in_selected_language') }}</li>
                    </ul>
                </div>
            </div>
        </div>
