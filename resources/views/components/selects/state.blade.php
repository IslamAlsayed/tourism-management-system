@php
    $name = isset($name) ? $name : 'state_id';
    $states = \Modules\Geography\Entities\State::orderBy('name')->get(['id', 'name']);
@endphp
<div>
    ...
    <select name="{{ isset($name) ? $name : '' }}" id="{{ isset($name) ? $name : '' }}" class="kt-select basic-single">
        ...
        @forelse ($states as $state)
            <option value="{{ $state->id }}"
                {{ isset($record->{isset($name) ? $name : ''}) && $record->{isset($name) ? $name : ''} == $state->id ? 'selected' : '' }}>
                {{ $state->name }}
            </option>
        @empty
