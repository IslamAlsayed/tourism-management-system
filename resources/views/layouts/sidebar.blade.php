...
<div class="kt-sidebar bg-background border-e border-e-border fixed top-0 bottom-0 z-2001 hidden lg:flex flex-col items-stretch shrink-0 [--kt-drawer-enable:true] lg:[--kt-drawer-enable:false]"
    ...
    {{-- Sidebar Header --}}
    <div class="kt-sidebar-header hidden lg:flex text-center justify-center relative px-3 lg:px-4 shrink-0 py-2" id="sidebar_header">
        <a class="dark:hidden flex items-center justify-center" href="{{ route('dashboard') }}">
            <img class="default-logo h-[4px] w-auto max-w-none" src="{{ $settings->app_light_photo ? asset('storage/' . $settings->app_light_photo) : asset('assets/images/logos/default-logo.svg') }}" />
            <img class="small-logo h-[45px] w-auto max-w-none" src="{{ $settings->app_mini_photo ? asset('storage/' . $settings->app_mini_photo) : asset('assets/images/logos/mini-logo.svg') }}" />
        </a>
        ...
        <button class="kt-btn kt-btn-outline kt-btn-icon size-[30px] absolute start-full top-2/4 -translate-x-2/4 -translate-y-2/4 rtl:translate-x-2/4" data-kt-toggle="body"
            data-kt-toggle-class="kt-sidebar-collapse" id="sidebar_toggle">
    ...
    <div class="kt-sidebar-content flex grow shrink-0 py-5" id="sidebar_content">
        <div class="kt-scrollable-y-hover grow shrink-0 flex ps-2 lg:ps-5 pe-1 lg:pe-3" style="padding-inline-end: 0" data-kt-scrollable="true" data-kt-scrollable-dependencies="#sidebar_header"
            data-kt-scrollable-height="auto" data-kt-scrollable-offset="0px" data-kt-scrollable-wrappers="#sidebar_content" id="sidebar_scrollable">
            ...
            <div class="kt-menu flex flex-col grow gap-1" data-kt-menu="true" id="sidebar_menu">
                ...
                @foreach ($menuItems as $item)
                    ...
                    @php
                        ...
                        $isActive = isset($item['route']) ? isActive($item['route'], $item['parameters'] ?? [], $currentRoute, $currentParameters) : false;
                        $hasActiveChild = $hasChildren ? hasActiveChild($item['children'], $currentRoute, $currentParameters) : false;

                        if ((isset(request()->type) && Str::contains($item['title'], request()->type)) || Str::contains($item['title'], singularLowerCaseName(key($currentParameters), '-'))) {
                            $hasActiveChild = true;
                        }
                        if (Str::contains(request()->segment(2), $item['title'])) {
                            $hasActiveChild = true;
                        }
                        if (Str::contains($item['title'], 'guides') && Str::contains(key($currentParameters), 'guides')) {
                            $hasActiveChild = true;
                        }
                    @endphp
                        ...
                        <div class="kt-menu-item {{ $hasActiveChild ? 'show' : '' }}" data-kt-menu-item-toggle="accordion" data-kt-menu-item-trigger="click">
                            <div
                                ...
                                <span class="kt-menu-icon items-start text-muted-foreground w-[20px]">
                                    <i class="{{ $item['icon'] ?? 'ki-filled ki-folder' }} {{ $hasActiveChild ? 'text-primary' : '' }}"></i>
                                </span>

                                <span class="kt-menu-title text-sm font-medium text-foreground {{ $hasActiveChild ? 'text-primary font-semibold' : '' }}">
                                    ...
                                    @if (isset($item['fixed']) && env('DB_MODE') != 'production')
                                        <span class="inline-block bg-primary/10 text-red-600 font-medium px-2 py-0.5 rounded-full ms-2" style="font-size: 10px">
                                ...
                                <span class="kt-menu-arrow text-muted-foreground shrink-0 justify-end ms-1 me-[-10px] {{ $hasActiveChild ? 'text-primary' : '' }}">
                            ...
                            <div
                                ...
                                @foreach ($item['children'] as $child)
                                    ...
                                    @php
                                        ...
                                        $childParameters = isset($child['parameters']) ? array_filter($child['parameters'], fn($key) => $key !== 't', ARRAY_FILTER_USE_KEY) : [];
                                        $currentParamsFiltered = array_filter($currentParameters, fn($key) => $key !== 't', ARRAY_FILTER_USE_KEY);

                                        $childIsActive = isActive($child['route'] ?? null, $childParameters, $currentRoute, $currentParamsFiltered);
                                        if (
                                            (isset(request()->type) && Str::contains($item['title'], request()->type) && Str::contains($currentRoute, $child['title'])) ||
                                            $child['title'] == str_replace('tours-', 'tour-', pluralLowerCaseName(str_replace('_', '-', key($currentParameters)), '-'))
                                        ) {
                                        ...
                                        // }
                                        if (
                                            Str::contains(request()->segment(3), str_replace('_', '-', key($currentParameters))) &&
                                    ...
                                    @if ($childHasChildren)
                                        {{-- Nested submenu --}}
                                        <div class="kt-menu-item {{ $childHasActiveChild ? 'kt-menu-item-show show' : '' }} px-2" data-kt-menu-item-toggle="accordion"
                                            ...
                                            <div
                                                ...
                                                @if (isset($child['icon']))
                                                    <span class="kt-menu-icon items-start text-muted-foreground w-[20px]">
                                                        <i class="{{ $child['icon'] }} {{ $childHasActiveChild ? 'text-primary' : '' }}"></i>
                                                    </span>
                                                @endif

                                                <span class="kt-menu-title text-sm font-medium text-foreground {{ $childHasActiveChild ? 'text-primary font-semibold' : '' }}">
                                                ...
                                                <span class="kt-menu-arrow text-muted-foreground shrink-0 justify-end ms-1 me-[-10px] {{ $childHasActiveChild ? 'text-primary' : '' }}">
                                                    <span class="inline-flex kt-menu-item-show:hidden">
                                                        <i class="ki-filled ki-plus text-[11px]"></i>
                                                    </span>
                                                    <span class="hidden kt-menu-item-show:inline-flex">
                                                ...
                                                @foreach ($child['children'] as $subChild)
                                                    ...
                                                    @php
                                                        ...
                                                        $subChildParameters = isset($subChild['parameters'])
                                                            ? array_filter($subChild['parameters'], fn($key) => $key !== 't', ARRAY_FILTER_USE_KEY)
                                                            : [];
                                                        $currentParamsFiltered = array_filter($currentParameters, fn($key) => $key !== 't', ARRAY_FILTER_USE_KEY);

                                                        $subChildIsActive = isActive($subChild['route'] ?? null, $subChildParameters, $currentRoute, $currentParamsFiltered);
                                                    @endphp
                                                    <div class="kt-menu-item px-2">
                                                        <a class="{{ $subChild['parameters']['types'] ?? '' }} kt-menu-link border border-transparent items-center grow {{ $subChildIsActive ? 'active bg-accent/60 rounded-[9px]' : '' }} hover:bg-accent/60 hover:rounded-[9px] gap-[14px] ps-[10px] pe-[10px] py-[8px]"
                                                            ...
                                                            <span class="kt-menu-icon items-start text-muted-foreground w-[10px]">
                                                                <i class="{{ $subChild['icon'] ?? 'ki-filled ki-folder' }} text-sm {{ $subChildIsActive ? 'text-primary' : '' }}"></i>
                                                            </span>

                                                            <span class="kt-menu-title text-2sm font-normal {{ $subChildIsActive ? 'text-primary font-semibold' : '' }}">
                                    ...
                                    @else
                                        ...
                                        <div class="kt-menu-item px-2">
                                            <a class="kt-menu-link border border-transparent items-center grow {{ $childIsActive ? 'active bg-accent/60 rounded-[9px]' : '' }} hover:bg-accent/60 hover:rounded-[9px] gap-[14px] ps-[10px] pe-[10px] py-[8px]"
                                                ...
                                                <span class="kt-menu-icon items-start text-muted-foreground w-[10px]">
                                                    <i class="{{ $child['icon'] ?? 'ki-filled ki-folder' }} text-sm {{ $childIsActive ? 'text-primary' : '' }}"></i>
                                                </span>

                                                <span class="kt-menu-title text-2sm font-normal {{ $childIsActive ? 'text-primary font-semibold' : '' }}">
                                                    {{ __('sidebar.' . $child['title']) }}
                    ...
                    @else
                        ...
                        <div class="kt-menu-item">
                            <a class="kt-menu-link mb-1 flex items-center grow border border-transparent gap-[10px] ps-[10px] pe-[10px] py-[6px] {{ $isActive ? 'active bg-accent/60' : '' }} hover:bg-accent/60 rounded-[9px] hover:rounded-[9px]"
                                ...
                                <span class="kt-menu-icon items-start text-muted-foreground w-[20px]">
                                    <i class="{{ $item['icon'] ?? 'ki-filled ki-folder' }} {{ $isActive ? 'text-primary' : '' }}"></i>
                                </span>

                                <span class="kt-menu-title text-sm font-medium {{ $isActive ? 'text-primary font-semibold' : '' }}">
                                    {{ __('sidebar.' . $item['title']) }}

                                    @if (isset($item['status']) && env('DB_MODE') != 'production')
                                        <span class="inline-block bg-primary/10 text-primary font-medium px-2 py-0.5 rounded-full ms-2" style="font-size: 10px">
                                            {{ __('sidebar.' . $item['status']) }}
                                        </span>
                                    @endif
