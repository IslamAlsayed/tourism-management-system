            ...
            <div class="kt-menu flex flex-col grow gap-1" data-kt-menu="true" data-kt-menu-accordion-expand-all="false"
                ...
                @foreach ($menuItems as $item)
                    @php
                        ...
                        $itemTitle = function_exists('getLocalizedText')
                                ...
                                <div
                                    ...
                                    @foreach ($item['children'] as $child)
                                        @php
                                            ...
                                            $childTitle = function_exists('getLocalizedText')
                                                ? getLocalizedText($child['title'])
                                                : (is_array($child['title'])
                                                    ? $child['title']['en'] ?? ''
                                                    : $child['title']);
                                                    ...
                                                    @foreach ($child['children'] as $subChild)
                                                        @php
                                                            $subChildIsActive =
                                                                isset($subChild['route']) &&
                                                                $currentRoute === $subChild['route'];
                                                            $subChildTitle = function_exists('getLocalizedText')
                                                                ? getLocalizedText($subChild['title'])
                                                                : (is_array($subChild['title'])
                                                                    ? $subChild['title']['en'] ?? ''
                                                                    : $subChild['title']);
                                                        @endphp
                ...
                @php
                    ...
                    $quickTitle = function_exists('getLocalizedText')
                        ? getLocalizedText($quickActions['title'])
                        : (is_array($quickActions['title'])
                            ? $quickActions['title']['en'] ?? 'Quick Tools'
                            : 'Quick Tools');
                @endphp
                ...
                @foreach ($quickActions['items'] as $quickItem)
                    @php
                        $quickItemTitle = function_exists('getLocalizedText')
                            ? getLocalizedText($quickItem['title'])
                            : (is_array($quickItem['title'])
                                ? $quickItem['title']['en'] ?? ''
                                : $quickItem['title']);
                    ...
                    <div class="kt-menu-item">
                        ...
                        @else
                            <a class="kt-menu-link flex items-center grow border border-transparent gap-[10px] ps-[10px] pe-[10px] py-[6px] hover:bg-accent/60 hover:rounded-lg {{ $quickItemClass }}"
                                href="{{ isset($quickItem['route']) ? route($quickItem['route']) : '#' }}"
                                tabindex="0">
                                <span class="kt-menu-icon items-start text-muted-foreground w-[20px]">
                                    <i class="{{ $quickItem['icon'] }} text-lg"></i>
                                </span>
