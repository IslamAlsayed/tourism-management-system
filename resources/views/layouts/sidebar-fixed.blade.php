        ...
        <div class="kt-scrollable-y-hover grow shrink-0 flex ps-2 lg:ps-5 pe-1 lg:pe-3" data-kt-scrollable="true"
            ...
            <div class="kt-menu flex flex-col grow gap-1" data-kt-menu="true" data-kt-menu-accordion-expand-all="false" id="sidebar_menu">
                                ...
                                <div class="kt-menu-accordion gap-1 ps-[10px] relative before:absolute before:start-[20px] before:top-0 before:bottom-0 before:border-s before:border-border">
                                    @foreach($item['children'] as $child)
                                        @php
                                            ...
                                            $childTitle = is_array($child['title']) ? ($child['title'][$currentLocale] ?? $child['title']['en'] ?? '') : $child['title'];
                                                ...
                                                <div class="kt-menu-accordion gap-1 ps-[10px] relative before:absolute before:start-[20px] before:top-0 before:bottom-0 before:border-s before:border-border">
                                                    @foreach($child['children'] as $subChild)
                                                        @php
                                                            ...
                                                            $subChildTitle = is_array($subChild['title']) ? ($subChild['title'][$currentLocale] ?? $subChild['title']['en'] ?? '') : $subChild['title'];
                                                        @endphp
                ...
                @php
                    ...
                    $quickTitle = is_array($quickActions['title']) ? ($quickActions['title'][$currentLocale] ?? $quickActions['title']['en'] ?? 'Quick Tools') : 'Quick Tools';
                @endphp
                ...
                @foreach($quickActions['items'] as $quickItem)
                    @php
                        $quickItemTitle = is_array($quickItem['title']) ? ($quickItem['title'][$currentLocale] ?? $quickItem['title']['en'] ?? '') : $quickItem['title'];
