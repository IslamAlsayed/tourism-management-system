...
<div class="kt-sidebar bg-background border-e border-e-border fixed top-0 bottom-0 z-999 hidden lg:flex flex-col items-stretch shrink-0 [--kt-drawer-enable:true] lg:[--kt-drawer-enable:false]"
    ...
    <div class="kt-sidebar-content flex grow shrink-0 py-5 pe-2 pt-0" id="sidebar_content">
            ...
            <div class="kt-menu flex flex-col grow gap-1" data-kt-menu="true" data-kt-menu-accordion-expand-all="false" id="sidebar_menu">
                ...
                @foreach ($menuItems as $item)
                    @php
                        ...
                        $itemTitle = is_array($item['title']) ? $item['title'][$currentLocale] ?? ($item['title']['en'] ?? '') : $item['title'];
                    ...
                    @if ($hasPermission)
                        @if ($hasChildren)
                            <!-- Menu with Children -->
                            <div class="kt-menu-item {{ $hasActiveChild ? 'kt-menu-item-show' : '' }}" data-kt-menu-item-toggle="accordion"
                                ...
                                <div
                                    ...
                                    @foreach ($item['children'] as $child)
                                        @php
                                            ...
                                            $childTitle = is_array($child['title'])
                                                ? $child['title'][$currentLocale] ?? ($child['title']['en'] ?? '')
                                                : $child['title'];
                                        ...
                                        @if ($childHasChildren)
                                            <!-- Nested submenu -->
                                            <div class="kt-menu-item {{ $childHasActiveChild ? 'kt-menu-item-show' : '' }}" data-kt-menu-item-toggle="accordion"
                                                data-kt-menu-item-trigger="click">
                                                <div class="kt-menu-link flex items-center grow cursor-pointer border border-transparent gap-[10px] ps-[10px] pe-[10px] py-[6px]"
                                                    tabindex="0">
                                                    @if (isset($child['icon']))
                                                ...
                                                <div
                                                    ...
                                                    @foreach ($child['children'] as $subChild)
                                                        @php
                                                            ...
                                                            $subChildTitle = is_array($subChild['title'])
                                                                ? $subChild['title'][$currentLocale] ?? ($subChild['title']['en'] ?? '')
                                                                : $subChild['title'];
                                                        @endphp
                                                        <div class="kt-menu-item">
                                                            <a class="kt-menu-link border border-transparent items-center grow {{ $subChildIsActive ? 'kt-menu-item-active:bg-accent/60' : '' }} hover:bg-accent/60 hover:rounded-lg gap-[14px] ps-[10px] pe-[10px] py-[8px]"
                                                                href="{{ isset($subChild['route']) ? route($subChild['route']) : '#' }}" tabindex="0">
                                                                <span
                                        ...
                                        @else
                                            ...
                                            <div class="kt-menu-item">
                                                <a class="kt-menu-link border border-transparent items-center grow {{ $childIsActive ? 'kt-menu-item-active:bg-accent/60' : '' }} hover:bg-accent/60 hover:rounded-lg gap-[14px] ps-[10px] pe-[10px] py-[8px]"
                                                    href="{{ isset($child['route']) ? route($child['route']) : '#' }}" tabindex="0">
                                                    <span
                ...
                @php
                    ...
                    $quickTitle = is_array($quickActions['title'])
                        ? $quickActions['title'][$currentLocale] ?? ($quickActions['title']['en'] ?? 'Quick Tools')
                        : 'Quick Tools';
                @endphp
                ...
                @foreach ($quickActions['items'] as $quickItem)
                    @php
                        $quickItemTitle = is_array($quickItem['title'])
                            ? $quickItem['title'][$currentLocale] ?? ($quickItem['title']['en'] ?? '')
                            : $quickItem['title'];
