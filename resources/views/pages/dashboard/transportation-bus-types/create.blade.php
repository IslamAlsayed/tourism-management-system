...
@section('title', __('main.add_new_type', ['type' => __('main.transportation_bus_type')]))
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <h1 class="text-xl font-medium leading-none text-mono">
                    {{ __('main.add_type', ['type' => __('main.transportation_bus_type')]) }}
                </h1>
                <div class="flex items-center gap-2 text-sm font-normal text-secondary-foreground">
                    {{ __('main.add_type_description', ['type' => __('main.transportation_bus_type')]) }}
                </div>
            </div>
            <div class="flex items-center gap-2.5">
                <a href="{{ route('transportation-bus-types.index') }}" class="kt-btn kt-btn-outline">
                    {{ __('main.back_to_types', ['type' => __('main.transportation_bus_types')]) }}
                </a>
            </div>
        </div>
    </div>
                ...
                <div class="kt-card-body">
                    <form method="POST" action="{{ route('transportation-bus-types.store') }}" class="space-y-6 p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 items-end mb-4">
                            ...
                            <div class="">
                                <label for="name"
                                    class="kt-label required mb-2">{{ __('main.transportation_bus_type') }}</label>
                        ...
                        <div class="flex items-center gap-4">
                            <button type="submit" class="kt-btn kt-btn-primary">
                                ...
                                {{ __('main.save_type', ['type' => __('main.transportation_bus_type')]) }}
                            </button>
