...
@section('title', __('main.edit_type', ['type' => __('main.meal_type')]))
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-4">
            <div class="flex flex-col justify-center gap-2">
                <h1 class="text-xl font-medium leading-none text-mono">
                    {{ __('main.edit_type', ['type' => __('main.meal_type')]) }}
                </h1>
                <div class="flex items-center gap-2 text-sm font-normal text-secondary-foreground">
                    {{ __('main.edit_type_description', ['type' => __('main.meal_type')]) }}
                </div>
            </div>
            ...
            <div class="kt-card-body">
                <form action="{{ route('accommodations.rates.update-meal', $mealRate->id) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6 mb-4">
                        ...
                        <div class="">
                            <label for="meal_type_id" class="kt-label required">{{ __('main.meal_type') }}</label>
                            <select class="kt-select @error('meal_type_id') is-invalid @enderror" id="meal_type_id"
                                name="meal_type_id" required>
                                <option value="">--</option>
                                @foreach ($mealTypes as $mealType)
                                    <option value="{{ $mealType->id }}"
                                        {{ $mealRate->meal_type_id == $mealType->id ? 'selected' : '' }}>
                                        {{ $mealType->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-4">
                        <div>
                            ...
                            <input type="number" step="0.01"
                                ...
                                value="{{ $mealRate->price }}" min="0" required>
                            @error('price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" maxlength="3"
                                ...
                                name="currency" value="{{ $mealRate->currency }}" placeholder="USD">
                            @error('currency')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
