...
@section('title', __('main.create_type', ['type' => __('main.accommodation-season')]))
...
@section('content')
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-4">
            <div class="flex flex-col justify-center gap-2">
                <h1 class="text-xl font-medium leading-none text-mono">
                    {{ __('main.create_type', ['type' => __('main.accommodation-season')]) }}
                </h1>
                <div class="flex items-center gap-2 text-sm font-normal text-secondary-foreground">
                    {{ __('main.create_type_description', ['type' => __('main.accommodation-season')]) }}
                </div>
            </div>
            <div class="flex items-center gap-2.5">
                <a href="{{ route('accommodations-seasons.index') }}" class="kt-btn kt-btn-outline">
                    {{ __('main.back_to_types', ['types' => __('main.accommodations-seasons')]) }}
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="kt-container-fixed">
        <form class="space-y-6" method="POST" action="{{ route('accommodations-seasons.store') }}">
            @csrf
            ...
            <!-- Save Submit Buttons -->
            @include('components.elements.save-submit', ['models' => 'accommodations-seasons'])
        </form>
    </div>
@endsection
