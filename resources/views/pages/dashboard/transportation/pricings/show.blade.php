...
@section('title', __('main.type_details', ['type' => __('main.transportation-pricing')]))
...
@section('content')
    <div class="kt-container-fixed">
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-4 pb-6">
            ...
            <div class="flex items-center gap-2.5">
                <a href="{{ route('transportation.pricings.edit', $pricing->id) }}" class="kt-btn kt-btn-primary md:hidden">
    ...
    <div class="kt-container-fixed">
        <div class="grid gap-4 lg:gap-6">
            <!-- Transportation Pricing Information -->
            <div class="kt-card">
                <div class="kt-card-header">
                    <h3 class="kt-card-title">
                        {{ __('main.type_information', ['type' => __('main.transportation-pricing')]) }}</h3>
                </div>
                            ...
                            <div wire:key="toggle-{{ $pricing->id }}-is_active">
                                ...
                                <div class="flex items-center gap-2">
                                    @livewire('toggle-switch', [
                                        'modelId' => $pricing->id,
                                        'modelType' => '\\App\\Models\\TransportationPricing',
                                        'field' => 'is_active',
                                        'value' => (bool) $pricing->is_active,
                                        'table' => 'transportation_pricings',
                                    ])
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-6">
                        @if ($pricing->creator)
                            <div>
                                ...
                                <p class="text-sm text-secondary-foreground">{{ $pricing->creator->name }}</p>
                            </div>
                        @endif
                        <div>
                            <label class="kt-label mb-1">{{ __('main.created_at') }}</label>
                            <p class="text-sm text-secondary-foreground">
                                {{ $pricing->created_at?->format('Y-m-d H:i:s') }}
                            </p>
                        </div>
                        @if ($pricing->updater)
                            <div>
                                <label class="kt-label mb-1">{{ __('main.updated_by') }}</label>
                                <p class="text-sm text-secondary-foreground">{{ $pricing->updater->name }}</p>
                            </div>
                        @endif
                        <div>
                            <label class="kt-label mb-1">{{ __('main.updated_at') }}</label>
                            <p class="text-sm text-secondary-foreground">
                                {{ $pricing->updated_at?->format('Y-m-d H:i:s') }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            @if ($pricing->vehicleType)
                @include('pages.dashboard.related-components.transportation-vehicle-type', [
