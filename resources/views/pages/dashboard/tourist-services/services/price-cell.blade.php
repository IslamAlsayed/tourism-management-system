...
<div class="flex flex-col gap-2">
    ...
    {{-- Commission Toggle (Uses toggle-hold style from Users page) --}}
    <div class="flex items-center justify-center gap-2 pt-1">
        <div class="toggle-hold mt-0">
            <input type="checkbox" id="{{ $uniqueId }}" name="{{ $prefix }}[{{ $type }}][{{ $nationalityKey }}][commission]" value="1"
                class="toggle-input commission-toggle" data-season="{{ $seasonId }}" data-type="{{ $type }}"
                data-nationality="{{ $nationalityKey }}" data-label="{{ ucfirst(str_replace('_', ' ', $type)) }} - {{ ucfirst($nationalityKey) }}"
                onchange="handleCommissionToggle(this)"
                {{ old($prefix . '[' . $type . '][' . $nationalityKey . '][commission]', $inputCommission ?? false) ? 'checked' : '' }}>
