...
@section('title', __('main.create_type', ['type' => __('main.jeep')]))
...
@section('content')
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-4">
            <div class="flex flex-col justify-center gap-2">
                <h1 class="text-xl font-medium leading-none text-mono">
                    {{ __('main.create_type', ['type' => __('main.jeep')]) }}
                </h1>
                <div class="flex items-center gap-2 text-sm font-normal text-secondary-foreground">
                    {{ __('main.create_type_description', ['type' => __('main.jeep')]) }}
                </div>
            </div>
    ...
    <div class="kt-container-fixed">
        <form action="{{ route('jeeps.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid gap-4 lg:gap-6">
                ...
                <div class="kt-card">
                    ...
                    <div class="kt-card-body p-4">
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
                            <!-- Provider Company -->
                            <div>
                                <label for="company_id" class="kt-label required">{{ __('main.transportation_company') }}</label>
                                <select name="company_id" id="company_id" class="kt-select basic-single" data-control="select2" required>
                            ...
                            <div class="col-span-full flex flex-col gap-4">
                                {{-- Route --}}
                                <div>
                                    <label for="route" class="kt-label required">
                                        {{ __('main.route') }}
                                        <span class="text-red-600 text-2xl">*</span>
                                    </label>
                                    <textarea name="route" id="route" class="kt-textarea" required>{{ old('route') }}</textarea>
                                    @error('route')
                                ...
                                {{-- Route Arabic --}}
                                <div>
                                    <label for="route_ar" class="kt-label">{{ __('main.route_ar') }}</label>
                                    <textarea name="route_ar" id="route_ar" class="kt-textarea">{{ old('route_ar') }}</textarea>
                                    @error('route_ar')
                                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-span-full">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <select name="origin_city_id" id="origin_city_id" class="kt-select basic-single cities-select" required>
                                    ...
                                    <div>
                                        ...
                                        <select name="destination_city_id" id="destination_city_id" class="kt-select basic-single cities-select" required>
                                            <option value="" selected>--</option>
                                        </select>
                            ...
                            <div class="col-span-full">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="flex gap-2">
                                        <div class="flex-grow">
                                            <label for="duration" class="kt-label required">{{ __('main.duration') }}</label>
                                            <input type="number" step="0.1" name="duration" id="duration" class="kt-input h-[45px]" value="{{ old('duration') }}" required>
                                        </div>
                                        <div class="w-1/3">
                                            <label for="duration_unit" class="kt-label required">{{ __('main.unit') }}</label>
                                            <select name="duration_unit" id="duration_unit" class="kt-select basic-single" required>
                                    ...
                                    <div class="flex gap-2">
                                        <div class="flex-grow">
                                            <label for="distance" class="kt-label required">{{ __('main.distance') }}</label>
                                            <input type="number" step="0.1" name="distance" id="distance" class="kt-input h-[45px]" value="{{ old('distance') }}" required>
                                        </div>
                                        <div class="w-1/3">
                                            <label for="distance_unit" class="kt-label required">{{ __('main.unit') }}</label>
                                            <select name="distance_unit" id="distance_unit" class="kt-select basic-single" required>
                                                @foreach ($distanceUnits as $key => $label)
                                                    <option value="{{ $key }}" {{ old('distance_unit') == $key ? 'selected' : '' }}>
                                                        {{ __('main.' . $key) }}
                                                    </option>
                                                @endforeach
                                            </select>
                    ...
                    <div class="kt-card-body p-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <input type="text" name="vehicle_model" class="kt-input h-[45px]" value="{{ old('vehicle_model') }}" placeholder="e.g. Toyota Land Cruiser">
                            </div>
                            <div>
                                <label class="kt-label">{{ __('main.model_year') }}</label>
                                <input type="text" name="model_year" class="kt-input h-[45px]" value="{{ old('model_year') }}" placeholder="e.g. 2025">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="kt-label">{{ __('main.license_plate') }}</label>
                                <input type="text" name="license_plate" class="kt-input h-[45px]" value="{{ old('license_plate') }}">
                            </div>
                            <div>
                                <label class="kt-label required">{{ __('main.seating_capacity') }}</label>
                                <input type="number" name="car_seats" class="kt-input h-[45px]" value="{{ old('car_seats', 4) }}" required min="1">
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="space-y-4">
                                    ...
                                    <div class="flex">
                                        <div class="w-full">
                                            <input type="number" step="0.01" name="price" id="price" class="h-full rounded-none bg-gray-50 text-gray-600 flex-1 min-w-0 w-full text-sm p-2.5"
                                                style="border: 1px var(--tw-border-style) var(--input)" placeholder="0.00" value="{{ old('price') }}">
                                        </div>
                                        ...
                                        <div class="w-[220px]">
                                            <select name="price_type"
                                                ...
                                                @foreach ($priceTypes as $key => $label)
                                                    <option value="{{ $key }}" {{ old('price_type', 'per_vehicle') == $key ? 'selected' : '' }}>
                                                        {{ __('main.' . $key) }}
                                                    </option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="grid grid-cols-2 items-end gap-4">
                                    ...
                                    <div class="w-full">
                                        ...
                                        <select name="status" id="status" class="kt-select basic-single w-full" required>
                                            <option value="" selected>--</option>
                    ...
                    <div class="season-item border rounded-md bg-gray-50 overflow-hidden" data-index="INDEX">
                        ...
                        <div class="p-4 season-body">
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                                <div class="w-full">
                                    <label class="block text-sm font-medium mb-1 required">{{ __('main.season_name') }}</label>
                                    <input type="text" name="seasons[INDEX][name]" class="kt-input h-[38px] season-name-input w-full" placeholder="Summer 2026" required>
                                </div>
                                <div class="w-full">
                                    <label class="block text-sm font-medium mb-1 required">{{ __('main.start_date') }}</label>
                                    <input type="date" name="seasons[INDEX][start_date]" class="kt-input h-[38px] w-full" required>
                                </div>
                                <div class="w-full">
                                    <label class="block text-sm font-medium mb-1 required">{{ __('main.end_date') }}</label>
                                    <input type="date" name="seasons[INDEX][end_date]" class="kt-input h-[38px] w-full" required>
                                </div>
                            </div>
                ...
                <template id="nationality-price-template">
                    <div class="flex gap-2 items-center nationality-price-row bg-gray-50 p-2 rounded border border-dashed border-gray-200">
                        <div class="w-full">
                            <select name="seasons[SEASON_INDEX][nationality_prices][NAT_INDEX][nationality_id]" class="kt-select nationality-select text-sm" id="nationality_id" required>
                                <option value="" selected>--</option>
                            </select>
                        </div>
                        <div class="w-full">
                        ...
                        <div class="w-full">
                            <input type="number" step="0.01" name="seasons[SEASON_INDEX][nationality_prices][NAT_INDEX][price]" class="kt-input h-[45px] text-sm"
                                placeholder="{{ __('main.price') }}" required>
                        </div>
                ...
                {{-- Description --}}
                @include('components.elements.input-text-editor', ['name' => 'description'])

                {{-- Notes --}}
                @include('components.elements.input-text-editor', ['name' => 'notes'])
                ...
                {{-- Save Buttons --}}
                @include('components.elements.save-submit', ['models' => 'jeeps'])
            </div>
        </form>
    </div>
@endsection
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function initNationalitySelect2($select) {
                ...
                $select.select2({
                    ajax: {
                        url: '{{ route('routes.nationalities') }}',
                        type: 'GET',
                        dataType: 'json',
                        delay: 250,
