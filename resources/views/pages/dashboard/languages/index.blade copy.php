                ...
                <table class="kt-table table-auto text-nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $language->name ?? '--' }}</td>
                            <td>
                                <img src="{{ $key <= 1 ? asset('metronic/media/flags/languages/' . $language->photo) : asset('storage/' . $language->photo) }}" alt="{{ $language->name }}" class="w-[30px]">
                            <td>
                                <span class="text-{{ $language->code == getCurrentLocale() ? 'green' : 'red' }}-600 font-semibold">
                            ...
                            <td class="px-4 py-2 text-end">
                                ...
                                <a href="{{ route('languages.change', $language->code) }}" class="kt-btn kt-btn-sm kt-btn-outline bg-green-500 text-white">
                                    {{ __('main.active') }}
                                </a>
                                @endif
                                <a href="{{ route('languages.edit', $language->id) }}" class="kt-btn kt-btn-sm kt-btn-outline bg-primary text-white">
                                    {{ __('main.edit') }}
                                </a>
                                <a href="{{ route('languages.destroy', $language->id) }}" class="kt-btn kt-btn-sm kt-btn-outline bg-danger text-white">
