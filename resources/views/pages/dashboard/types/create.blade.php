...
@section('title', __('main.add_new_type', ['type' => __('main.type')]))
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-4 pb-6">
            <div class="flex flex-col justify-center gap-2">
                <h1 class="text-xl font-medium leading-none text-mono">
                    {{ __('main.add_type', ['type' => __('main.type')]) }}
                </h1>
                <div class="flex items-center gap-2 text-sm font-normal text-secondary-foreground">
                    {{ __('main.add_type_description', ['type' => __('main.type')]) }}
                </div>
            </div>
            <div class="flex items-center gap-2.5">
                <a href="{{ route('types.index') }}" class="kt-btn kt-btn-outline">
                    {{ __('main.back_to_types', ['type' => __('main.types')]) }}
                </a>
            </div>
        </div>
    </div>
        ...
        <div class="grid gap-4 lg:gap-6">
            ...
            <div class="kt-card">
                ...
                <div class="kt-card-body">
                    <form method="POST" action="{{ route('types.store') }}" class="space-y-6 p-4">
                        @csrf
                        ...
                        <div class="flex items-center gap-4">
                            <button type="submit" class="kt-btn kt-btn-primary">
                                ...
                                {{ __('main.save_type', ['type' => __('main.type')]) }}
                            </button>
