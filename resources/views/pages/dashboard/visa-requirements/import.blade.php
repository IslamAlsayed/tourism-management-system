...
<div class="container mx-auto px-4 py-6 max-w-7xl" x-data="{ uploadMethod: 'local' }">
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex gap-2">
             <a href="{{ route('export.data', ['models' => 'visa-requirements']) }}" class="px-3 py-2 rounded-lg border border-gray-200 text-xs font-bold hover:bg-gray-50 bg-white transition-colors flex items-center gap-2">
                <i class="ki-filled ki-file-up"></i> {{ __('main.export') }}
            </a>
            <a href="{{ route('visa-requirements.index') }}" class="px-3 py-2 rounded-lg bg-gray-100 text-gray-700 text-xs font-bold hover:bg-gray-200 transition-colors flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="flex shadow-sm rounded-lg">
                            <input type="number" placeholder="0.00" class="flex-1 w-full text-xs font-medium border border-gray-300 rounded-s-lg focus:border-primary focus:ring-primary border-e-0 h-10 px-3">
                            <select class="w-24 text-xs font-medium border border-gray-300 bg-gray-50 text-gray-600 rounded-e-lg focus:border-primary focus:ring-primary border-s h-10 px-2">
                                <option>USD</option>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative">
                            <input type="number" placeholder="Days" class="w-full text-xs font-medium border border-gray-300 rounded-lg focus:border-primary focus:ring-primary h-10 pl-3 pr-10">
                    ...
                    <div class="col-span-1 md:col-span-2 lg:col-span-3 space-y-1.5">
                        ...
                        <textarea placeholder="Add any specific conditions, notes, or descriptions here..." class="w-full text-xs font-medium border border-gray-300 rounded-lg focus:border-primary focus:ring-primary h-10 py-2 px-3 resize-none bg-white"></textarea>
                    </div>

                    <!-- Submit Button -->
                    <div class="flex items-end">
                        <button type="submit" class="w-full h-10 bg-gray-900 hover:bg-black text-white rounded-lg text-xs font-bold transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 border border-black">
    ...
    <form action="{{ route('import.data.post', ['models' => 'visa-requirements']) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-5">
            ...
            <!-- Step 1: Download -->
            <div class="lg:col-span-3 kt-card border border-gray-200 shadow-sm bg-white rounded-lg p-3 flex flex-col justify-between h-full hover:border-blue-400 transition-colors">
                <div>
                ...
                <button type="button" onclick="downloadTemplate()" class="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded border border-dashed border-blue-300 text-blue-600 text-[10px] font-bold hover:bg-blue-50 transition-all">
            ...
            <!-- Step 2: Upload -->
            <div class="lg:col-span-6 kt-card border border-gray-200 shadow-sm bg-white rounded-lg p-3 h-full flex flex-col hover:border-blue-400 transition-colors">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="flex bg-gray-100 rounded p-0.5 border border-gray-200">
                        <button type="button" @click="uploadMethod = 'local'" :class="uploadMethod === 'local' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500'" class="px-2 py-0.5 text-[10px] font-bold rounded transition-all">Local</button>
                        <button type="button" @click="uploadMethod = 'url'"   :class="uploadMethod === 'url'   ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500'" class="px-2 py-0.5 text-[10px] font-bold rounded transition-all">Link</button>
                    </div>
                </div>

                <!-- Local -->
                <div x-show="uploadMethod === 'local'" class="flex-grow flex flex-col justify-center">
                    <label class="flex flex-col items-center justify-center w-full h-16 border border-dashed border-gray-300 rounded cursor-pointer hover:bg-gray-50 hover:border-primary transition-all group bg-gray-50/50">
                         <div class="flex items-center gap-2">
                            <i class="ki-filled ki-cloud-add text-lg text-gray-400 group-hover:text-primary transition-colors"></i>
                            <span class="text-[10px] text-gray-600 font-medium group-hover:text-primary transition-colors">{{ __('main.drop_files_here') }}</span>
                         </div>
                        <input name="file" type="file" class="hidden" accept=".csv,.xlsx" onchange="handleFileSelect(this)" />
                    </label>
                    <div id="file_name_container" class="mt-1 hidden text-[10px] flex items-center gap-1 text-blue-700 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">
                ...
                <div x-show="uploadMethod === 'url'" style="display: none;" class="flex-grow flex flex-col justify-center">
                    <div class="relative">
                        ...
                        <input type="url" name="file_url" placeholder="Paste Google Drive / Direct Link" class="w-full ps-7 pe-2 py-1.5 text-[11px] border border-gray-300 rounded focus:border-blue-500 outline-none" />
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-3 kt-card border border-gray-200 shadow-sm bg-white rounded-lg p-3 flex flex-col justify-between h-full hover:border-blue-400 transition-colors">
                <div>
                    ...
                    <div class="space-y-1.5 mb-2">
                        <label class="flex items-center gap-2 cursor-pointer select-none">
                            <input type="checkbox" name="has_header" value="1" class="size-3 rounded border-gray-300 text-blue-600 focus:ring-0" checked>
                ...
                <button type="submit" class="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded bg-blue-600 text-white text-[10px] font-bold hover:bg-blue-700 transition-all shadow-sm border border-transparent">
                    {{ __('main.import_now') }} 
                    ...
                    <thead class="bg-gray-50 text-[9px] text-gray-500 uppercase tracking-tight">
                        <tr class="h-8">
                            ...
                            <th class="px-2 py-1.5 border-b border-e border-gray-100 last:border-e-0">
                                ...
                                @if($col['req']) <span class="text-red-500">*</span> @endif
                            </th>
                            @endforeach
