...
class TransportationRouteSeeder extends Seeder
{
    ...
    private function createRouteAssignments($route)
    {
        ...
        $companies = TransportationCompany::where('is_active', true)->limit(3)->get();
        $currency = Currency::where('code', 'EGP')->first() ?? Currency::first();

        foreach ($companies as $company) {
            // جلب أنواع المركبات للشركة
            $vehicleTypes = TransportationVehicleType::where('company_id', $company->id)->where('is_active', true)->limit(2)->get();

            if ($vehicleTypes->isEmpty()) {
                $vehicleTypes = TransportationVehicleType::where('is_active', true)->limit(2)->get();
            }
            ...
            foreach ($vehicleTypes as $vehicleType) {
                ...
                TransportationRouteAssignment::create([
                    'route_id' => $route->id,
