...
@section('title', __('main.edit_vessel'))
...
@section('content')
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-4">
            <div class="flex flex-col justify-center gap-2">
                <h1 class="text-xl font-medium leading-none text-mono">
                    {{ __('main.edit_vessel') }} - {{ $cruise->name }}
                </h1>
                <div class="flex items-center gap-2 text-sm font-normal text-secondary-foreground">
                    {{ __('main.edit_vessel_description') }}
                </div>
            </div>
            <div class="flex items-center gap-2.5">
                ...
                <a href="{{ route('dashboard.cruises.show', $cruise) }}" class="kt-btn kt-btn-light">
    ...
    <div class="container-fixed">
        <form class="space-y-6" method="POST" action="{{ route('dashboard.cruises.update', $cruise) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid gap-4 lg:gap-6">
                <!-- Cruise Photo -->
                @include('components.input-image', ['record' => $cruise, 'column' => 'cruise', 'columnName' => 'photo'])
                ...
                <div class="kt-card">
                    ...
                    <div class="kt-card-body p-4">
                        @livewire('geography::livewire.regions.location-select-base2', ['model' => $cruise])
                ...
                <div class="kt-card">
                    ...
                    <div class="kt-card-body p-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Main Start Point -->
                            @php
                                $startCountryId = old('start_country_id', $cruise->main_start_point_id ? (\Modules\Geography\Entities\City::find($cruise->main_start_point_id)->country_id ?? '') : '');
                                $endCountryId = old('end_country_id', $cruise->main_end_point_id ? (\Modules\Geography\Entities\City::find($cruise->main_end_point_id)->country_id ?? '') : '');
                            @endphp
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="kt-input-group">
                                        ...
                                        <select id="start_country_id" name="start_country_id" class="kt-input" data-control="select2">
                                            ...
                                            @foreach($countries as $country)
                                                <option value="{{ $country->id }}" {{ $startCountryId == $country->id ? 'selected' : '' }}>
                                    ...
                                    <div class="kt-input-group">
                                        ...
                                        <select name="main_start_point_id" id="main_start_point_id" class="kt-input" data-control="select2" disabled>
                                            ...
                                            @foreach($cities as $city)
                                                <option value="{{ $city->id }}" data-country="{{ $city->country_id }}" {{ old('main_start_point_id', $cruise->main_start_point_id) == $city->id ? 'selected' : '' }} style="display: none;">
                                ...
                                <div>
                                    ...
                                    <div class="kt-input-group">
                                        ...
                                        <select id="end_country_id" class="kt-input" data-control="select2">
                                            <option value="" selected disabled>{{ __('main.select_option') }}</option>
                                            @foreach($countries as $country)
                                                <option value="{{ $country->id }}" {{ $endCountryId == $country->id ? 'selected' : '' }}>
                                                    {{ app()->getLocale() == 'ar' ? $country->name_ar : $country->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div>
                                    ...
                                    <div class="kt-input-group">
                                        ...
                                        <select name="main_end_point_id" id="main_end_point_id" class="kt-input" data-control="select2" disabled>
                                            <option value="" selected disabled>{{ __('main.select_option') }}</option>
                                            @foreach($cities as $city)
                                                <option value="{{ $city->id }}" data-country="{{ $city->country_id }}" {{ old('main_end_point_id', $cruise->main_end_point_id) == $city->id ? 'selected' : '' }} style="display: none;">
                                                    {{ app()->getLocale() == 'ar' ? $city->name_ar : $city->name }}
                                                </option>
                                            @endforeach
                                        </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <input type="text" name="name" id="name" class="kt-input" value="{{ old('name', $cruise->name) }}" required>
                                </div>
                                @error('name')
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <input type="text" name="name_ar" id="name_ar" class="kt-input" value="{{ old('name_ar', $cruise->name_ar) }}">
                                </div>
                                @error('name_ar')
                                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                                @enderror
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <select name="vessel_class" id="vessel_class" class="kt-input">
                                        ...
                                        <option value="Standard" {{ old('vessel_class', $cruise->vessel_class) == 'Standard' ? 'selected' : '' }}>Standard</option>
                                        <option value="Luxury" {{ old('vessel_class', $cruise->vessel_class) == 'Luxury' ? 'selected' : '' }}>Luxury</option>
                                        <option value="Mega" {{ old('vessel_class', $cruise->vessel_class) == 'Mega' ? 'selected' : '' }}>Mega</option>
                                    </select>
                                </div>
                                @error('vessel_class')
                                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <select name="type" id="type" class="kt-input">
                                        <option value="" disabled>{{ __('main.select_option') }}</option>
                                        <option value="Ocean" {{ old('type', $cruise->type) == 'Ocean' ? 'selected' : '' }}>Ocean</option>
                                        <option value="River" {{ old('type', $cruise->type) == 'River' ? 'selected' : '' }}>River</option>
                                    </select>
                                </div>
                                @error('type')
                                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <select name="vessel_nationality_id" id="vessel_nationality_id" class="kt-input" data-control="select2">
                                        <option value="" selected disabled>{{ __('main.select_option') }}</option>
                                        @foreach($countries as $country)
                                            <option value="{{ $country->id }}" {{ old('vessel_nationality_id', $cruise->vessel_nationality_id) == $country->id ? 'selected' : '' }}>
                                                {{ app()->getLocale() == 'ar' ? $country->name_ar : $country->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <input type="number" name="total_cabins" id="total_cabins" class="kt-input" value="{{ old('total_cabins', $cruise->total_cabins) }}" min="0">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <input type="number" name="deck_count" id="deck_count" class="kt-input" value="{{ old('deck_count', $cruise->deck_count) }}" min="0">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <input type="number" name="built_year" id="built_year" class="kt-input" value="{{ old('built_year', $cruise->built_year) }}" min="1900" max="{{ date('Y') }}">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <input type="number" name="renovated_year" id="renovated_year" class="kt-input" value="{{ old('renovated_year', $cruise->renovated_year) }}" min="1900" max="{{ date('Y') }}">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <input type="number" step="0.01" name="length_meters" id="length_meters" class="kt-input" value="{{ old('length_meters', $cruise->length_meters) }}" min="0">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="kt-input-group">
                                    ...
                                    <input type="number" step="0.01" name="draft_meters" id="draft_meters" class="kt-input" value="{{ old('draft_meters', $cruise->draft_meters) }}" min="0">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="kt-card-body p-4 flex gap-6">
                        <div class="flex items-center gap-3">
                            ...
                            @include('components.elements.checkbox-button', [
                                ...
                                'id' => 'is_active',
                                'value' => '1',
                                'checked' => old('is_active', $cruise->is_active),
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            @include('components.elements.checkbox-button', [
                                ...
                                'id' => 'is_chartered',
                                'value' => '1',
                                'checked' => old('is_chartered', $cruise->is_chartered),
                ...
                <div class="flex justify-end gap-3 mt-4">
                    ...
                    <button type="submit" class="kt-btn kt-btn-primary">{{ __('main.save_changes') }}</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        function setupLocationDependency(countrySelectId, citySelectId) {
            ...
            countrySelect.on('change', function(e, isInit) {
        ...
        setTimeout(() => {
            if ($('#start_country_id').data('select2')) {
                $('#start_country_id').trigger('change', [true]);
                $('#end_country_id').trigger('change', [true]);
            }
        }, 500);
    });
</script>
