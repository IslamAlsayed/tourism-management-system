...
@section('title', __('main.create_type', ['type' => __('main.cruise-supplier')]))
...
@section('content')
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-4">
            <div class="flex flex-col justify-center gap-2">
                <h1 class="text-xl font-medium leading-none text-mono">
                    {{ __('main.create_type', ['type' => __('main.cruise-supplier')]) }}
                </h1>
            </div>
    ...
    <div class="container-fixed">
        <form class="space-y-6" method="POST" action="{{ route('dashboard.cruises.suppliers.store') }}">
            @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="type" id="type" class="kt-input h-[45px]" required>
                                    ...
                                    <option value="ship_owner" {{ old('type') === 'ship_owner' ? 'selected' : '' }}>{{ __('main.ship_owner') }}</option>
                                    <option value="broker" {{ old('type') === 'broker' ? 'selected' : '' }}>{{ __('main.broker') }}</option>
                                    <option value="agency" {{ old('type') === 'agency' ? 'selected' : '' }}>{{ __('main.agency') }}</option>
                                    <option value="operator" {{ old('type') === 'operator' ? 'selected' : '' }}>{{ __('main.operator') }}</option>
                                </select>
                                @error('type')
                                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="default_commission_rate" id="default_commission_rate"
                                    class="kt-input h-[45px]" value="{{ old('default_commission_rate') }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="date" name="contract_start_date" id="contract_start_date"
                                    class="kt-input h-[45px]" value="{{ old('contract_start_date') }}">
                            ...
                            <div>
                                ...
                                <input type="date" name="contract_end_date" id="contract_end_date"
                                    class="kt-input h-[45px]" value="{{ old('contract_end_date') }}">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="payment_terms" id="payment_terms" class="kt-input h-24 py-3"
                                    rows="3" placeholder="{{ __('main.payment_terms_placeholder') }}">{{ old('payment_terms') }}</textarea>
                                @error('payment_terms')
                                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
