            ...
            <table class="w-full text-left" style="min-w: 800px;">
                <thead>
                    <tr class="bg-gray-50 dark:bg-dark-card border-b border-gray-200 dark:border-gray-800">
                        <th class="p-4 text-xs font-semibold uppercase text-gray-500 w-48 sticky left-0 bg-gray-50 dark:bg-dark-card z-10 border-r border-gray-200 dark:border-gray-800">
                            {{ __('main.season') }} / {{ __('main.cabin_category') }}
                        </th>
                        @foreach ($categories as $category)
                            <th class="p-4 text-xs font-semibold uppercase text-gray-500 text-center border-r border-gray-200 dark:border-gray-800">
                                {{ $category->name }}
                                <span class="block text-[10px] text-primary lowercase font-normal">({{ $category->code }})</span>
                            </th>
                        @endforeach
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                    @foreach ($seasons as $season)
                        <tr class="hover:bg-gray-50 dark:hover:bg-dark-card transition-colors">
                            <td class="p-4 sticky left-0 bg-white dark:bg-dark-card z-10 border-r border-gray-200 dark:border-gray-800">
                                <span class="text-sm font-bold text-gray-800 dark:text-gray-200 block">{{ $season->name }}</span>
                            ...
                            @foreach ($categories as $category)
                                <td class="p-4 border-r border-gray-100 dark:border-gray-800/50 align-top">
                                    <div class="grid grid-cols-1 gap-3">
                                        <!-- Buy Price (cost) -->
                                        <div class="relative">
                                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-gray-400 uppercase">Buy</span>
                                            <input type="number" 
                                        ...
                                        <!-- Sell Price (Retail) -->
                                        <div class="relative">
                                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-success uppercase">Sell</span>
                                            <input type="number" 
