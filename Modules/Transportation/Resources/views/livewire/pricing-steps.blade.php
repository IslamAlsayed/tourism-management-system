    ...
    <div class="align-self-end {{ !hasEmpty($filters['company']) ? 'disabled-option rounded-sm' : '' }}">
        <label for="season_id" class="kt-label mb-2 flex items-center justify-between">
            <div>
                ...
                <i class="i-loader fas fa-refresh fa-spin text-primary" wire:loading wire:target="filters.company,updatedFilters"></i>
                <span id="season_id-info" class="text-red-600 text-sm span-info {{ !hasEmpty($filters['company']) ? 'show' : '' }}">
                    ({{ __('main.select_type_first', ['type' => __('main.company')]) }})
                </span>
            </div>
        ...
        <select name="season_id" id="season_id" class="kt-select basic-single" {{ !hasEmpty($filters['company']) ? 'disabled' : '' }}>
            ...
            @foreach ($options['seasons'] as $item)
                <option value="{{ $item->id }}" {{ old('season_id', $record->season_id ?? null) == $item->id ? 'selected' : '' }}>
                    {{ $item->name }}</option>
            @endforeach
        </select>
        @error('season_id')
