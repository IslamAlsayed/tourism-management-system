...
@section('content')
    ...
    <div class="kt-container-fixed">
        <form action="{{ route('dashboard.transportation.pricings.update', $pricing->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="kt-card-body p-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 items-end gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="price" id="price" class="kt-input h-[45px]" value="{{ $pricing->price }}" minlength="1">
                                @error('price')
                            ...
                            <div>
                                ...
                                <input type="number" name="tax" id="tax" class="kt-input h-[45px]" value="{{ $pricing->tax }}" minlength="0"
                                    maxlength="100">
                                @error('tax')
                                    <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="pricing_unit_id" id="pricing_unit_id" class="kt-input basic-single">
                                    ...
                                    @foreach ($pricingUnits as $unit)
                                        <option value="{{ $unit->id }}" {{ $pricing->pricing_unit_id == $unit->id ? 'selected' : '' }}>
                                            {{ $unit->name }}
                                        </option>
                                    @endforeach
                                </select>
