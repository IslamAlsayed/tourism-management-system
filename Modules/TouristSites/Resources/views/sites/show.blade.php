    ...
    <div class="container-fixed">
        <div class="grid gap-4 lg:gap-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        ...
                        @if ($touristSite->city)
                            <div>
                                ...
                                <a href="{{ route('dashboard.geography.cities.show', $touristSite->city->id) }}"
            ...
            <div class="kt-card">
                ...
                <div class="kt-card-body p-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        @if ($touristSite->country_id || $touristSite->city?->country)
                            @php
                                $country = $touristSite->country_id
                                    ? \Modules\Geography\Entities\Country::find($touristSite->country_id)
                                    : $touristSite->city?->country;
                            @endphp
                            @if ($country)
                                <div>
                                    <label class="kt-label mb-1">{{ __('main.country') }}</label>
                                    <a href="{{ route('dashboard.geography.countries.show', $country->id) }}"
                                        class="block text-sm text-primary underline">
                                        {{ $country->name }}
                                        <i class="fa-duotone fa-solid fa-arrow-up-right-from-square ms-1"></i>
                                    </a>
                            ...
                            @endif
                        @endif
                        @if ($touristSite->state_id || $touristSite->city?->state)
                            @php
                                $state = $touristSite->state_id
                                    ? \Modules\Geography\Entities\State::find($touristSite->state_id)
                                    : $touristSite->city?->state;
                            @endphp
                            @if ($state)
                                <div>
                        ...
                        <div>
                            ...
                            <p class="text-sm text-secondary-foreground">{{ $touristSite->postal_code ?: __('main.na') }}
                            </p>
                        </div>
                        @include('components.elements.displayable-rich-text', [
                            'record' => $touristSite,
            ...
            <!-- Entry Fees -->
            @if (
                $touristSite->is_free_entry ||
                    $touristSite->entry_fee_adult ||
                    $touristSite->entry_fee_child ||
                    $touristSite->entry_fee_foreigner_adult)
                <div class="kt-card">
                    <div class="kt-card-header">
                            ...
                            @foreach (['entry_fee_adult', 'entry_fee_child', 'entry_fee_student', 'entry_fee_senior', 'entry_fee_group', 'entry_fee_foreigner_adult', 'entry_fee_foreigner_child', 'entry_fee_arab_adult', 'entry_fee_arab_child', 'entry_fee_local_adult', 'entry_fee_local_child', 'entry_fee_resident_adult', 'entry_fee_resident_child'] as $fee)
                                @if ($touristSite->$fee)
                                    <div>
                                        ...
                                        <p class="text-sm text-secondary-foreground">{{ $touristSite->$fee }}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        ...
                        @if (!$touristSite->is_24_7)
                            <div>
                                ...
                                <p class="text-sm text-secondary-foreground">
                                    {{ $touristSite->opening_time ?: __('main.na') }}</p>
                            </div>
                            <div>
                                <label class="kt-label mb-1">{{ __('main.closing_time') }}</label>
                                <p class="text-sm text-secondary-foreground">
                                    {{ $touristSite->closing_time ?: __('main.na') }}</p>
                            </div>
                        @endif
                                ...
                                <div class="flex flex-wrap" style="gap: 10px 20px;">
                                    @foreach ($hours as $hourDate => $time)
                                        <div class="border-custom p-2 rounded-md">
                                            ...
                                            @else
                                                <span>{{ $time['opening'] ?? __('main.na') }} -
                                                    {{ $time['closing'] ?? __('main.na') }}</span>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        @if ($touristSite->local_guide_price)
                            <div>
                                ...
                                <p class="text-sm text-secondary-foreground">{{ $touristSite->local_guide_price }}
                                    {{ $touristSite->currency?->code ?? '' }}</p>
                            </div>
                        @endif
                        @if ($touristSite->club_car_price)
                            <div>
                                ...
                                <p class="text-sm text-secondary-foreground">{{ $touristSite->club_car_price }}
                                    {{ $touristSite->currency?->code ?? '' }}</p>
                            </div>
                        @endif
                        ...
                        @if ($touristSite->estimated_visit_duration)
                            <div>
                                ...
                                <p class="text-sm text-secondary-foreground">{{ $touristSite->estimated_visit_duration }}
                                    {{ __('main.minutes') }}</p>
                            </div>
                        @endif
                        @if ($touristSite->difficulty_level)
                            <div>
                                ...
                                <p class="text-sm text-secondary-foreground">
                                    {{ __('main.' . strtolower($touristSite->difficulty_level)) }}</p>
                            </div>
                        @endif
                        @if ($touristSite->tags)
                            <div class="col-span-full">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @php
                                        $tags = is_array($touristSite->tags)
                                            ? $touristSite->tags
                                            : json_decode($touristSite->tags, true) ?? [];
                                    @endphp
                                    @foreach ($tags as $tag)
                                        <span
                        ...
                        @if ($touristSite->status)
                            <div>
                                ...
                                <select class="kt-input kt-input-sm basic-single disabled-option js-async-field"
                                    data-model="touristSites" data-id="{{ $touristSite->id }}" data-field="status">
                ...
                try {
                    ...
                    window.showToast({
                        ...
                        message: data.message ||
                            `{{ __('messages.field_updated_successfully', ['field' => ':field', 'status' => ':status']) }}`
                            .replace(
                                ':field', payload
