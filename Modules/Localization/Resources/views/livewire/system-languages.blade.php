        ...
        @if ($view == 'grid')
            <div class="kt-cards p-4" wire:key="{{ $view ? $view : '' }}-view">
                <div class="inline-flex text-nowrap items-center gap-2 text-center mb-2 cursor-pointer">
                    ...
                    {{-- Grid length --}}
                    <div>
                        <select wire:change="toggleGridLength($event.target.value,'system_languages')" wire:model.live="gridLength" class="kt-select">
            ...
            <div wire:key="{{ $view ? $view : '' }}-view" data-kt-datatable="true" data-kt-datatable-state-save="false" id="team_crew_table">
                <div class="kt-scrollable-x-auto" wire:loading.class="loading"
                    ...
                    <table class="kt-table table-auto text-nowrap">
                        ...
                        <tbody>
                            @foreach ($data as $language)
                                <tr wire:key="{{ $language->id }}" class="{{ getCurrentLocale() == $language->code ? 'bg-gray-100' : 'hover:bg-gray-100' }}">
                                    ...
                                    <td class="px-4 py-2 text-end">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            @else
                                                <span style="padding-inline: 17px" class="kt-btn kt-btn-sm bg-yellow-500 text-white">
                                                    {{ __('main.currently') }}
                                                </span>
                                            @endif
                                            @include('components.elements.edit-button', [
