...
@section('content')
    ...
    <div class="kt-container-fixed">
        <form action="{{ route('dashboard.geography.nationalities.update', $nationality->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid gap-4 lg:gap-6">
                    ...
                    <div class="kt-card-body p-4">
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
                            ...
                            <div class="">
                                ...
                                <select name="country_id" id="country_id" class="kt-input basic-single">
                                    @foreach ($countries as $country)
                                        <option value="{{ $country->id }}" {{ $nationality->country_id == $country->id ? 'selected' : '' }}>
                                            {{ $country->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="">
                                ...
                                <input type="text" name="name" id="name" class="kt-input h-[45px]" value="{{ $nationality->name }}">
                                @error('name')
                                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="">
                                ...
                                <input type="text" name="name_ar" id="name_ar" class="kt-input h-[45px]" value="{{ $nationality->name_ar }}">
                                @error('name_ar')
                                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <!-- Update Submit -->
                @include('components.elements.update-submit', ['models' => 'dashboard.geography.nationalities', 'model' => 'nationality'])
            </div>
        </form>
    </div>
@endsection
