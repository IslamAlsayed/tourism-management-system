...
@section('title', __('main.edit_type', ['type' => __('main.field-definition')]))
...
@section('content')
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-4">
            <div class="flex flex-col justify-center gap-2">
                <h1 class="text-xl font-medium leading-none text-mono">
                    {{ __('main.edit_type', ['type' => __('main.field-definition')]) }}: {{ $fieldDefinition->name }}
                </h1>
    ...
    <div class="kt-container-fixed">
        <form action="{{ route('dashboard.core.field-definitions.update', $fieldDefinition->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid gap-4 lg:gap-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                            ...
                            <div class="align-self-end">
                                ...
                                <input type="text" class="kt-input h-[45px]" id="name" name="name"
                                    value="{{ old('name', $fieldDefinition->name) }}" required>
                                @error('name')
                            ...
                            <div class="align-self-end">
                                ...
                                <input type="text" class="kt-input h-[45px]" id="name_ar" name="name_ar"
                                    value="{{ old('name_ar', $fieldDefinition->name_ar) }}" dir="rtl">
                                @error('name_ar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="align-self-end">
                                ...
                                <select name="module_name" id="module_name" class="kt-input basic-single h-[45px]" required
                                    ...
                                    @foreach ($modules as $modKey => $modData)
                                        <option value="{{ $modKey }}"
                                            {{ old('module_name', $fieldDefinition->module_name) == $modKey ? 'selected' : '' }}>
                                            {{ $modData['label'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="align-self-end">
                                ...
                                <select name="field_type" id="field_type" class="kt-input basic-single h-[45px]" required
                                    ...
                                    @foreach ($fieldTypes as $ftKey => $ftLabel)
                                        <option value="{{ $ftKey }}"
                                            {{ old('field_type', $fieldDefinition->field_type) == $ftKey ? 'selected' : '' }}>
                                            {{ $ftLabel }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="align-self-end">
                                ...
                                <input type="number" class="kt-input h-[45px]" id="sort_order" name="sort_order"
                                    value="{{ old('sort_order', $fieldDefinition->sort_order) }}" min="0">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="kt-card" id="options_section"
                    style="{{ in_array($fieldDefinition->field_type, ['select', 'multiselect', 'radio']) ? '' : 'display: none;' }}">
                    <div class="kt-card-header">
                    ...
                    <div class="kt-card-body p-4">
                        ...
                        <textarea class="kt-input" id="options" name="options" rows="3">{{ old('options', is_array($fieldDefinition->options) ? implode(', ', $fieldDefinition->options) : $fieldDefinition->options) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" class="kt-input h-[45px]" id="placeholder" name="placeholder"
                                    value="{{ old('placeholder', $fieldDefinition->placeholder) }}">
                            </div>
                            <div>
                                ...
                                <input type="text" class="kt-input h-[45px]" id="placeholder_ar"
                                    name="placeholder_ar"
                                    value="{{ old('placeholder_ar', $fieldDefinition->placeholder_ar) }}" dir="rtl">
                            </div>
                            <div>
                                ...
                                <input type="text" class="kt-input h-[45px]" id="section_label" name="section_label"
                                    value="{{ old('section_label', $fieldDefinition->section_label) }}">
                            </div>
                            <div>
                                ...
                                <input type="text" class="kt-input h-[45px]" id="section_label_ar"
                                    ...
                                    value="{{ old('section_label_ar', $fieldDefinition->section_label_ar) }}"
                                    dir="rtl">
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex flex-wrap" style="gap: 10px 40px;">
                    <div class="flex items-center gap-3">
                        ...
                        @include('components.elements.checkbox-button', [
                            ...
                            'id' => 'is_required',
                            'value' => '1',
                            'checked' => old('is_required', $fieldDefinition->is_required),
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @include('components.elements.checkbox-button', [
                            ...
                            'id' => 'is_active',
                            'value' => '1',
                            'checked' => old('is_active', $fieldDefinition->is_active),
                ...
                @include('components.elements.save-submit', [
                    'models' => 'dashboard.core.field-definitions',
...
@push('scripts')
    <script>
        ...
        function toggleOptions() {
            ...
            document.getElementById('options_section').style.display = needsOptions ? 'block' : 'none';
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            updateEntityOptions();
            // Set the old entity value after populating options
            setTimeout(() => {
                const entitySelect = document.getElementById('entity_type');
                entitySelect.value = '{{ old('entity_type', $fieldDefinition->entity_type) }}';
                if (typeof $ !== 'undefined' && $(entitySelect).data('select2')) {
                    $(entitySelect).trigger('change.select2');
                }
            }, 100);
