    ...
    <div class="container-fixed">
        <div class="grid gap-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
                ...
                <div class="kt-card h-fit {{ $settings->app_google_maps_key ? '' : 'bg-yellow-100' }}">
                    ...
                    <div class="kt-card-body">
                        <div class="space-y-6 p-4">
                            <div class="flex items-center justify-between gap-3">
                                        ...
                                        <div>
                                            <div class="font-semibold">
                                                ...
                                                @if (in_array(getActiveUser()->role, ['admin', 'superadmin']))
                                                    <span class="font-semibold text-primary underline cursor-pointer" toggle-button style="user-select: none;"
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-2">
                                        <div class="kt-badge kt-badge-{{ $settings->app_google_maps_key ? 'success' : 'danger' }}">
                            ...
                            @if (in_array(getActiveUser()->role, ['admin', 'superadmin']))
                                <form method="POST" action="{{ route('dashboard.core.settings.update', $settings->id) }}" class="mt-4 hidden"
                                    id="google-maps-api">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="kt-card">
                                        <div class="kt-card-body p-6">
                                            <div class="space-y-6">
                                                <div>
                                                    ...
                                                    <input type="text" name="app_google_maps_key" class="kt-input h-[45px]"
                                        ...
                                        <div class="flex items-center justify-start gap-4 px-6 pb-6">
                                            <button type="submit" class="kt-btn kt-btn-primary" wire:confirm="Are you sure you want to save these changes?">
                                            ...
                                            <span class="kt-btn bg-danger" onclick="document.getElementById('google-maps-api')?.classList.add('hidden');">
                                                @if (isset(getActiveUser()->button_display_mode) && getActiveUser()->button_display_mode === 'text')
                                                    {{ __('main.cancel') }}
                                                @elseif (isset(getActiveUser()->button_display_mode) && getActiveUser()->button_display_mode === 'icon')
                ...
                <div class="kt-card h-fit" style="background: var(--color-{{ $settings->app_ably_key ? '' : 'yellow' }}-100);">
                    <div class="kt-card-header">
                        ...
                        <div class="space-y-6 p-4">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <div>
                                    <label class="kt-label mb-2">{{ __('main.status') }}</label>
                                    <div class="flex items-center gap-2">
                                        <div class="kt-badge kt-badge-{{ $settings->app_ably_key ? 'success' : 'danger' }}">
                            ...
                            @if (in_array(getActiveUser()->role, ['admin', 'superadmin']))
                                <form method="POST" action="{{ route('dashboard.core.settings.update', $settings->id) }}" class="mt-4 hidden" id="ably-key">
                                    @csrf
                                    @method('PUT')
            ...
            {{-- Email (SMTP) Settings --}}
            <form method="POST" action="{{ route('dashboard.core.settings.update', $settings->id) }}" id="smtp-settings-form">
                @csrf
                @method('PUT')
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="app_smtp_host" class="kt-input h-[45px]"
                                        value="{{ $settings->app_smtp_host ?? config('mail.mailers.smtp.host') }}" placeholder="smtp.gmail.com" />
                                </div>

                                <div>
                                    <label class="kt-label mb-2">{{ __('main.smtp_port') }}</label>
                                    <input type="number" name="app_smtp_port" class="kt-input h-[45px]"
                                        value="{{ $settings->app_smtp_port ?? config('mail.mailers.smtp.port') }}" placeholder="587" />
                                </div>

                                <div>
                                    <label class="kt-label mb-2">{{ __('main.smtp_username') }}</label>
                                    <input type="text" name="app_smtp_username" class="kt-input h-[45px]"
                                        value="{{ $settings->app_smtp_username ?? config('mail.mailers.smtp.username') }}" placeholder="your-email@gmail.com" />
                                </div>

                                <div>
                                    <label class="kt-label mb-2">{{ __('main.smtp_password') }}</label>
                                    <input type="text" name="app_smtp_password" class="kt-input h-[45px]"
                                        value="{{ $settings->app_smtp_password ?? config('mail.mailers.smtp.password') }}" placeholder="••••••••" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <form method="POST" action="{{ route('dashboard.core.settings.update', $settings->id) }}" id="notification-channels-form">
                @csrf
                @method('PUT')
                    ...
                    <div class="kt-card-body">
                        <div class="space-y-4 p-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                                ...
                                <div class="disabled">
                                    <div class="kt-card p-4 {{ $settings->app_sms_notifications == 1 ? '' : 'bg-yellow-100' }}">
                                        <div class="flex items-center justify-between">
                                            <div>
            ...
            {{-- Notification Types --}}
            <form method="POST" action="{{ route('dashboard.core.settings.update', $settings->id) }}" id="notification-types-form" class="disabled-option">
                @csrf
                @method('PUT')
                    ...
                    <div class="kt-card-body">
                        <div class="space-y-4 p-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                                ...
                                <div class="disabled">
                                    <div class="kt-card p-4 {{ $settings->app_notifications_system_reports == 1 ? '' : 'bg-yellow-100' }}">
                                        <div class="flex items-center justify-between">
                                            <div>
                                ...
                                <div class="disabled">
                                    <div class="kt-card p-4 {{ $settings->app_notifications_security_updates == 1 ? '' : 'bg-yellow-100' }}">
                                        <div class="flex items-center justify-between">
                                            <div>
            ...
            <div class="disabled-option">
                <div class="kt-card" id="test-email-card">
                    ...
                    <div class="kt-card-body">
                        <form action="{{ route('web-push-notifications') }}" method="POST" class="p-6" id="web-push-form">
                            @csrf
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="subject" class="kt-input h-[45px]" placeholder="Test Subject" value="صباح الخير" />
                            </div>
