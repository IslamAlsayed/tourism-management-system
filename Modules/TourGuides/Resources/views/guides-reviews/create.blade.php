...
@section('title', __('main.create_type', ['type' => __('main.tours.guide-review')]))
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-4 pb-6">
            <div class="flex flex-col justify-center gap-2">
                <h1 class="text-xl font-medium leading-none text-mono">
                    {{ __('main.create_type', ['type' => __('main.tours.guide-review')]) }}
                </h1>
                <div class="flex items-center gap-2 text-sm font-normal text-secondary-foreground">
                    {{ __('main.create_type_description', ['type' => __('main.tours.guide-review')]) }}
                </div>
            </div>
    ...
    <div class="kt-container-fixed">
        <div class="kt-card p-4">
            <div class="kt-card-body">
                <form class="space-y-6" method="POST" action="{{ route('dashboard.tourguides.guides-reviews.store') }}">
                    @csrf
                            ...
                            <div class="">
                                ...
                                <select name="tour_guide_id" id="tour_guide_id" class="kt-select basic-single">
                                    ...
                                    @foreach ($tourGuides as $tourGuide)
                                        <option value="{{ $tourGuide->id }}" {{ old('tour_guide_id') == $tourGuide->id ? 'selected' : '' }}>
                                            {{ $tourGuide->name }}
                                        </option>
                                    @endforeach
                                </select>
