...
class AirportController extends Controller
{
    ...
    public function store(StoreRequest $request)
    {
        ...
        return $created
            ? ($request->has('save_and_add')
                ...
                : redirect()->route('dashboard.entrypoints.airports.index')->with('success', __('messages.type_created', ['type' => __('main.airport')])))
            : redirect()->route('dashboard.entrypoints.airports.index')->with('error', __('messages.type_creation_failed', ['type' => __('main.airport')]));
    }
    ...
    public function update(UpdateRequest $request, $id)
    {
        ...
        return $updated
            ? redirect()->route('dashboard.entrypoints.airports.index')->withSuccess(__('messages.type_updated', ['type' => __('main.airport')]))
    ...
    public function destroy($id)
    {
        ...
        return $deleted
            ? redirect()->route('dashboard.entrypoints.airports.index')->withSuccess(__('messages.type_deleted', ['type' => __('main.airport')]))
            : redirect()->route('dashboard.entrypoints.airports.index')->withError(__('messages.type_deletion_failed', ['type' => __('main.airport')]));
    }
    ...
    public function filtered($filtered)
    {
        ...
        if (!isset($validTypes[$filtered]))
            return redirect()->route('dashboard.entrypoints.airports.index')->withError(__('messages.invalid_type'));
