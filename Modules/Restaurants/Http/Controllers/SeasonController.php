...
class SeasonController extends Controller
{
    ...
    {
        return view('restaurants::seasons.index');
    }

    public function create()
    {
        $restaurants = Restaurant::orderBy('name')->get(['id', 'name']);
        return view('restaurants::seasons.create', compact('restaurants'));
    }
    ...
    public function store(StoreRequest $request)
    {
        ...
        return $created
            ? ($request->has('save_and_add')
                ...
                : redirect()->route('dashboard.restaurants.seasons.index', ['type' => $request->input('type')])->with('success', __('messages.type_created', ['type' => __('main.season')])))
            : redirect()->route('dashboard.restaurants.seasons.index', ['type' => $request->input('type')])->with('error', __('messages.type_creation_failed', ['type' => __('main.season')]));
    }
    ...
    public function show($id)
    {
        ...
        return view('restaurants::seasons.show', compact('season'));
    }
    ...
    public function edit($id)
    {
        ...
        if (!$season)
            return redirect()->back()->withError(__('messages.not_found_this_type', ['type' => __('main.season')]));
        $restaurants = Restaurant::orderBy('name')->get(['id', 'name']);
        return view('restaurants::seasons.edit', compact('season', 'restaurants'));
    }
    ...
    public function update(UpdateRequest $request, $id)
    {
        ...
        return $updated
            ? redirect()->route('dashboard.restaurants.seasons.index', ['type' => $request->input('type')])->withSuccess(__('messages.type_updated', ['type' => __('main.season')]))
    ...
    public function destroy($id)
    {
        ...
        return $deleted
            ? redirect()->route('dashboard.restaurants.seasons.index')->withSuccess(__('messages.type_deleted', ['type' => __('main.season')]))
            : redirect()->route('dashboard.restaurants.seasons.index')->withError(__('messages.type_deletion_failed', ['type' => __('main.season')]));
    }
}
